...
ShortestPaths::ShortestPaths(
    const TransitionRewirer &rewirer, const vector<int> &costs,
    ...
    : rewirer(rewirer),
      ...
      task_has_zero_costs(
          any_of(costs.begin(), costs.end(), [](int c) { return c == 0; })),
      num_parents(0) {
...
void ShortestPaths::resize(int num_states) {
    ...
    if (use_cache && num_parents > max_cached_parents) {
        log << "Maximum number of cached shortest paths exceeded --> clear cache."
            << endl;
...
void ShortestPaths::recompute(
    const Abstraction &abstraction, const Goals &goals) {
    open_queue.clear();
    ...
    while (!open_queue.empty()) {
        ...
        for (const Transition &t :
             abstraction.get_incoming_transitions(state_id)) {
...
unique_ptr<Solution> ShortestPaths::extract_solution(
    ...
    while (!goals.count(current_state)) {
        ...
        assert(
            states[t.target_id].goal_distance <=
            states[current_state].goal_distance);
...
void ShortestPaths::add_parent(int state, const Transition &new_parent) {
    ...
    assert(
        find(parents[state].begin(), parents[state].end(), new_parent) ==
        parents[state].end());
    ...
    assert(
        find(
            target_children.begin(), target_children.end(),
...
void ShortestPaths::update_incrementally(
    ...
    */
    ...
    states[v1].goal_distance = states[v2].goal_distance =
        states[v].goal_distance;
    ...
    */
    ...
    while (!candidate_queue.empty()) {
        ...
        if (use_cache) {
            ...
            for (const Transition &t :
                ...
                if (!states[succ].dirty &&
                    add_costs(
                        states[succ].goal_distance, operator_costs[op_id]) ==
                        states[state].goal_distance) {
                    if (debug) {
                        cout << "Reconnect " << state << " to " << succ
                             << " via " << op_id << " with cost "
            ...
            if (use_cache) {
                ...
                for (const Transition &t : children[state]) {
                    ...
                    if (!states[prev].dirty_candidate && !states[prev].dirty) {
                        if (debug) {
                            log << "Add " << prev << " to candidate queue"
                                << endl;
                        }
        ...
        if (timer.is_expired()) {
            // Up to here all goal distances are always lower bounds, so we can
            // abort at any time.
    ...
    while (!open_queue.empty()) {
        ...
        for (const Transition &t :
            ...
            if (states[succ].dirty && succ_g < states[succ].goal_distance) {
                assert(
                    count(dirty_states.begin(), dirty_states.end(), succ) == 1);
...
bool ShortestPaths::is_optimal_transition(
    int start_id, int op_id, int target_id) const {
    return states[start_id].goal_distance - operator_costs[op_id] ==
           states[target_id].goal_distance;
}
...
bool ShortestPaths::test_distances(
    ...
    vector<int> init_distances(
        num_states, 0); // Don't compute reachability info.
    ...
    for (int v = 0; v < num_states; ++v) {
        ...
        if (use_cache) {
            ...
            for (const Transition &parent : parents[v]) {
                ...
                assert(
                    count(
                        children[w].begin(), children[w].end(),
                        Transition(op_id, v)) == 1);
            ...
            assert(
                states[v].goal_distance ==
                add_costs(
                    operator_costs[t.op_id],
                    states[t.target_id].goal_distance));
        }
    }

    vector<int> goal_distances_32_bit =
        compute_goal_distances(abstraction, costs, goals);
    ...
    for (int i = 0; i < num_states; ++i) {
        if (goal_distances_32_bit_rounded_down[i] != goal_distances_32_bit[i] &&
            ...
            log << "32-bit rounded:   " << goal_distances_32_bit_rounded_down
                << endl;
