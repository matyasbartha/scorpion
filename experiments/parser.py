...
def find_cegar_max_expansion_message(content, props):
    props["max_expansion_with_flaw"] = content.count(
        "Max expansions reached with flaws!"
    )
    props["max_expansion_without_flaw"] = content.count(
        "Max expansions reached with no flaw!"
    )
...
def get_parser():
    ...
    parser.add_pattern(
        "num_state_expansions_in_flaw_search",
        r"\] #Expanded concrete states: (.+)\n",
        type=int,
    )

    parser.add_pattern(
        "max_num_state_expansions_in_flaw_search",
        r"\] #Max expanded concrete states in one flaw search: (.+)\n",
        type=int,
    )
    ...
    parser.add_pattern(
        "flaw_search_time",
        r"\] Flaw search time: (.+)s\n",
        type=float,
    )

    parser.add_pattern(
        "split_computing_time",
        r"\] Time for computing splits: (.+)s\n",
        type=float,
    )

    parser.add_pattern(
        "pick_computing_time",
        r"\] Time for selecting splits: (.+)s\n",
        type=float,
    )

    parser.add_pattern(
        "additive_cartesian_heuristic_build_time",
